...
This document outlines best practices for taking applications generated or enhanced by Kybernus to production.

## 🚀 The Fast Path: `kybernus deploy`

The easiest way to prepare your app for production is using the `deploy` command. It generates the necessary configuration files for your chosen provider.
...
**Supported Providers:**
- **Vercel**: Optimized for Next.js, Express, and FastAPI.
- **Railway**: Uses `railway.toml` for easy backend scaling.
- **Fly.io**: Generates `fly.toml` and an optimized `Dockerfile`.
- **Render**: Generates `render.yaml` for Blueprint deployments.
...
Optimized `Dockerfile` configurations can be generated during `init` or via `kybernus deploy`.
*   **Production Build**: Use multi-stage Docker builds to minimize image size and exclude `devDependencies`.
*   **Networking**: Ensure production overrides securely configure ports and do not expose database ports publicly.

### Database Management
Templates include robust database integrations (Prisma, SQLAlchemy, Spring Data JPA).
...
If you selected "GitHub Actions", your repo contains a pipeline in `.github/workflows/`.
